...
app.post('/register', (req, res) => {
  ...
  const role = lEmail.endsWith('@mcgill.ca') && !lEmail.endsWith('@mail.mcgill.ca')
    ? 'owner'
    : 'student';
...
app.post('/slots/create', (req, res) => {
  ...
  const result = db.prepare(
    'INSERT INTO slots (owner_id, date, time, type, active, created_at) VALUES (?, ?, ?, ?, 1, ?)'
...
app.post('/bookings', async (req, res) => {
  ...
  try {
    const result = db.prepare(
      'INSERT INTO bookings (student_id, slot_id, created_at) VALUES (?, ?, ?)'
    ).run(student_id, slot_id, nowIso());
...
app.delete('/pins/:studentId/:ownerId', (req, res) => {
  db.prepare('DELETE FROM pinned_profs WHERE student_id = ? AND owner_id = ?').run(
    req.params.studentId, req.params.ownerId,
  );
